...
output "databricks_service_principal_terraform_plan_details" {
  ...
  value = {
    application_id   = var.service_principal_name_terraform_plan == "" ? "" : one(data.databricks_service_principal.service_principal_terraform_plan[*].application_id)
    acl_principal_id = var.service_principal_name_terraform_plan == "" ? "" : one(data.databricks_service_principal.service_principal_terraform_plan[*].acl_principal_id)
  }
  sensitive = false
}
